...
func cmd() *cobra.Command {
	...
	c := cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if rootCAs == "" {
				cli, _ := newK8sClient()
				se, err := cli.CoreV1().Secrets("kube-system").Get("dex.ca.tls", v1.GetOptions{})
				if err != nil {
					log.Println(err.Error())
				} else {
					if debug {
						log.Println("ca.pem")
						log.Println(se.Data["ca.pem"])
					}
					_ = ioutil.WriteFile("/tmp/k8s-ca.pem", se.Data["ca.pem"], 0755)
				}
				rootCAs = "/tmp/k8s-ca.pem"
			}
	...
	c.Flags().StringVar(&a.clientID, "client-id", "dex-login", "OAuth2 client ID of this application.")
	...
	c.Flags().StringVar(&issuerURL, "issuer", "https://172.27.139.108:32000", "URL of the OpenID Connect issuer.")
